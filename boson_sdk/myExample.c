...
int main()
{
	...
	devh = libusb_open_device_with_vid_pid(ctx, 2507, 16391); //these are vendorID and productID
	if(devh == NULL)
	{
		printf("Failed to open Boson!\n");
		libusb_exit(ctx);
		return -1;
	}
	printf("CAMERA IS OPEN!!!\n");

	result = Initialize(devh);
